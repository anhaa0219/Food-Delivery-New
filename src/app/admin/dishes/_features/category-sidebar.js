...
export const CategorySideBar = () => {
  ...
  const foodCategoryPost = async () => {
    try {
      const response = await server.post("/foodCategory/post", {
  ...
  const foodCategoryPostDelete = async (id) => {
    console.log(id);
    try {
      const response = await server.delete("/foodCategory/delete", {
        data: {
          id: id,
        },
      });
  ...
  return (
    <div className="w-full mt-15 flex-col gap-4 py-6 px-6 min-h-59 flex bg-[#FFFFFF] rounded-xl ">
      ...
      <div className="w-full flex flex-wrap gap-3">
          ...
          return (
            ...
            >
              <div
                ...
                onClick={(e) => {
                  e.preventDefault();
                  foodCategoryPostDelete(data._id);
                }}
              >
                <X className="w-2 h-2" />
              </div>
        ...
        {plus && (
          <div
            className="w-115 h-68 flex flex-col rounded-xl bg-[#FFFFFF] p-6
            gap-6
          ...
          >
            ...
            <div className="flex items-end justify-end w-103 h-16">
              <div
                ...
                className="w-30.75 h-10 bg-[#18181B] rounded-md flex justify-center items-center font-inter font-medium text-[14px] leading-5 text-[#FAFAFA] cursor-pointer"
              >
                Add category
              </div>
            </div>
